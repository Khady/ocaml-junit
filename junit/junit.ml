...
module Testcase = struct
  ...
  let make ~name ~classname ~time result = { name; classname; time; result }

  let error ?message ~typ ~name ~classname ~time description =
    let result = Error { message; typ; description } in
    make ~name ~classname ~time result
  ;;

  let failure ?message ~typ ~name ~classname ~time description =
    let result = Failure { message; typ; description } in
    make ~name ~classname ~time result
  ;;

  let skipped ~name ~classname ~time = make ~name ~classname ~time Skipped
  let pass ~name ~classname ~time = make ~name ~classname ~time Pass
end
...
module Testsuite = struct
  ...
  let make ?package ?timestamp ?(hostname = "localhost") ?system_out ?system_err ~name () =
    let package =
      match package with
      | None -> name
      | Some p -> p
  ...
  let add_testcase testcase t =
    ...
    | Testcase.Pass | Testcase.Skipped -> t
    | Testcase.Error _ -> { t with errors = t.errors + 1 }
    | Testcase.Failure _ -> { t with failures = t.failures + 1 }
  ;;
  ...
  let add_property properties t = { t with properties = properties :: t.properties }
...
let make testsuites =
  let testsuites = List.mapi (fun i t -> Testsuite.{ t with id = i }) testsuites in
  let length = List.length testsuites in
  { next_id = length; testsuites }
;;

let add_testsuite testsuite t =
  let ts = Testsuite.{ testsuite with id = t.next_id } in
  { next_id = succ t.next_id; testsuites = ts :: t.testsuites }
;;
    ...
    List.map
      (fun t ->
        let properties =
          ...
          List.map
            (fun p -> Junit_xml.property ~name:p.name ~value:p.value)
        ...
        let testcases =
          ...
          List.map
            (fun p ->
              Junit_xml.testcase ~name:p.name ~classname:p.classname ~time:p.time p.result)
